...
const STRIKE_BONUS_PER = 0.5; // % of base per strike
...
export function getPendingNormalTodos() {
  return db.prepare(`
    SELECT * FROM todos
    WHERE status = 'pending' AND (task_type IS NULL OR task_type = 'normal')
...
export function awardXp(baseXp = XP_PER_TASK, strikeBonus = 0) {
  const profile = getChildProfile();
  ...
  return { gained, xp: newXp, level: newLevel, leveled_up: newLevel > profile.level };
}
...
export function onTodoCompleted(todo) {
  ...
  const xpResult = awardXp(XP_PER_TASK, strikeBonus);
